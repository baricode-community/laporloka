        ...
        <div class="container mx-auto px-4">
            ...
            <div class="text-center mb-12">
                <h1 class="text-4xl font-extrabold text-gray-900 mb-4">
                    Semua <span class="bg-gradient-to-r from-blue-600 to-green-600 bg-clip-text text-transparent">Laporan Warga</span>
                </h1>
                <p class="text-lg text-gray-600 max-w-2xl mx-auto">
                    Transparansi untuk lingkungan yang lebih baik. Pantau setiap aspirasi dan pengaduan yang masuk secara real-time.
                </p>
            </div>

            <div class="bg-white rounded-2xl shadow-xl shadow-blue-100/50 border border-gray-100 p-6 mb-12 max-w-5xl mx-auto">
                <form method="GET" action="{{ route('reports.index') }}">
                    <div class="grid gap-5 md:grid-cols-12">
                        ...
                        <div class="md:col-span-2">
                            <button type="submit" class="w-full h-full px-6 py-3 bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition-all duration-200 font-semibold shadow-lg shadow-blue-200 flex items-center justify-center gap-2">
                                <span>Filter</span>
                            </button>
                        </div>
                    </div>
                    ...
                    @if ($search || $categoryFilter || $statusFilter)
                        <div class="mt-4 flex justify-end">
                            <a href="{{ route('reports.index') }}" class="text-sm text-red-500 hover:text-red-700 font-medium flex items-center gap-1 transition-colors">
            ...
            @if ($reports->count() > 0)
                <div class="mb-6 text-sm text-gray-500 font-medium ml-1">
                    Menampilkan {{ $reports->firstItem() }}-{{ $reports->lastItem() }} dari total {{ $reports->total() }} laporan
                </div>
                        ...
                        <div onclick="window.location.href='{{ route('reports.show', $report->id) }}'"
                            ...
                            <div class="flex justify-between items-start mb-4">
                                <div class="flex items-center gap-2">
                                    @if ($report->category)
                                        <span class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold bg-gray-50 text-gray-600 border border-gray-100">
                            ...
                            <div class="mb-6">
                                <h3 class="text-lg font-bold text-gray-900 mb-2 group-hover:text-blue-600 transition-colors line-clamp-2 leading-tight">
                                    {{ $report->title }}
                                </h3>
                            ...
                            <div class="flex items-center justify-between pt-4 border-t border-gray-100">
                                <div class="flex items-center gap-2">
                                    <div class="w-8 h-8 bg-gradient-to-br from-blue-500 to-blue-600 rounded-full flex items-center justify-center text-white text-xs font-bold shadow-md">
                                        {{ substr($report->user->name ?? 'A', 0, 1) }}
                                    </div>
                                    <div class="flex flex-col">
                                        <span class="text-xs font-bold text-gray-700">{{ $report->user->name ?? 'Anonim' }}</span>
            ...
            @else
                <div class="text-center py-20 bg-white rounded-3xl border border-dashed border-gray-200">
                    <div class="bg-gray-50 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6">
                        <svg class="w-10 h-10 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m2 6H7a2 2 0 01-2-2V6a2 2 0 012-2h5l2 2h5a2 2 0 012 2v10a2 2 0 01-2 2z"></path></svg>
                    </div>
                    ...
                    <a href="{{ route('reports.index') }}" class="text-blue-600 font-semibold hover:underline">Refresh Halaman</a>
                </div>
            @endif
        </div>
    </div>
